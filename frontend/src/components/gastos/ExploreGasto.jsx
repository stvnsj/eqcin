...
const create_operation = async function (
    option,
    empleado_option,
    gasto_proyecto,
    gasto_monto,
    ...
    const data ={
        ...
        fecha        : date0,
        comentario   : gasto_comentario,
        contrato     : gasto_contrato.id,
    };

    var url = ""

    if (option == OPTION.BONO){url = `http://localhost:8000/bono`;} 
    else if (option == OPTION.ANTICIPO){url = `http://localhost:8000/anticipo`;} 
    else if (option == OPTION.DESCUENTO){url = `http://localhost:8000/descuento`;}
    ...
    try{
        ...
        toast.access("Operación Realizada");
...
const init_table = async function (
    ...
    var url = ""
    if (option == OPTION.BONO){url = `http://localhost:8000/bono/trabajador/${id}`;} 
    else if (option == OPTION.ANTICIPO){url = `http://localhost:8000/anticipo/trabajador/${id}`;} 
    else if (option == OPTION.DESCUENTO){url = `http://localhost:8000/descuento/trabajador/${id}`;}
                ...
                return (
                    <ThemeProvider theme={theme}>
                        <Button
                            ...
                            color='yellow'
                            onClick={(e) => {
				                const element = contrato_options.find(item => item.id === params.row.contrato_id);
				                ...
				                set_gasto_contrato(element)
				                set_gasto_fecha(new Date(params.row.datetime))
				                console.log(params.row.fecha)
                                set_option(UI_OPTION.anexo);        
                            }}
                            variant="contained"
                        >
                            editar
    ...
    return(
        ...
        <>
...

            {
                ...
                (option==UI_OPTION["nuevo"] && empleado_option) ?
                <div style={{backgroundColor: '#eeeeee',
                    ...
                    <Typography>Crear {expense_name()}</Typography>
                    <br/>
                ...
                : (option==UI_OPTION["anexo"] && empleado_option) ?
                <div style={{backgroundColor: '#eeeeee',
                             borderRadius: "10px",
                             display: "inline-block",
                             border: '1px solid #000',
                ...
                : (option==UI_OPTION.termino && empleado_option) ?
                <div style={{backgroundColor: '#eeeeee',
                             borderRadius: "10px",
                             display: "inline-block",
                             border: '1px solid #000',
