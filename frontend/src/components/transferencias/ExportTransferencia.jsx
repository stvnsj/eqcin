...
export default function ExportTransferencia(){
  ...
  const filenameBuilder  =  () => {
    ...
    return `Transferencias_${timeRange}`;
  }
  ...
  useEffect(()=>{
    ...
    const url = `http://localhost:8000/transferencia/xport/${year}`;
    ...
    .then(blob => {
      ...
      a.href = url;
      a.download = `transferencias_${year}` + '.xlsx' ;
      a.click();
  ...
  useEffect(()=>{
    ...
    const url = `http://localhost:8000/transferencia/detalle/${date1}/${date2}/${option}`;
