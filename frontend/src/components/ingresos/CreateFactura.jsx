...
export default function CreateFactura(props) {
    ...
    const post_data = async function () {
        ...
        if(!rut)  {toast.error("El rut del cliente no puede estar vacío"); return;}
        if(!razon)  {toast.error("La razón social del cliente no puede estar vacía"); return;}
        ...
        const url = 'http://localhost:8000/ingreso/factura/create';


        try{
            
            const res = await axios.post(url,data);            
            toast.success("Factura ingresada");
            clear_data();
    ...
    return(
        ...
        <>
            ...
            <br/><br/>
            <TxtField value={rut} setter={setRut} label={"Rut Cliente"}/>
            <br/><br/>
            <TxtField value={razon} setter={setRazon} label={"Razon Social Cliente"}/>
            <br/><br/>
            <TextField
                value={comentario}
