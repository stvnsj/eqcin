...

{
    ...
    return (
      <ThemeProvider theme={theme}>
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            toggle_activo(params.row.id, props.proyectoID);
          }}
        >
  ...
  React.useEffect(()=>{
    ...
    const toggle_activo = async (empleado_id, proyecto_id) => {
        try {
            const url = `http://localhost:8000/proyecto/toggle-activo/${empleado_id}/${proyecto_id}`;
            const res = await axios.get(url);

            const nuevoActivo = !!res.data.activo;
            ...
            props.notify("No se pudo cambiar el estado", "error");
        }
        };
