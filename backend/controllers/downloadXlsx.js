...
const {analisis_categorias} = require("../repositories/index")
...
exports.resumen_anual_xlsx = async (req, res, next) => {
    ...
    `;
        ...
        for (const row of reportRows) {
            ...
            PROYECTO_IDS.push({"id": row.id, "nombre": row.nombre})
        ...
        for (const p of PROYECTO_IDS) {

            const proyecto_sheet = workbook.addWorksheet(`${p["nombre"]}`);
            const proyecto_data = await analisis_categorias(p["id"])


            console.log("DATOS");
            console.log(proyecto_data);

            proyecto_sheet.getCell(`A1`).value = `${proyecto_data[0].categoria_total}`;
